...
locale mereology  =
  fixes part:: "'a \<Rightarrow> 'a \<Rightarrow> bool" (infix "\<preceq>" 50)

begin

definition proper_part:: "'a \<Rightarrow> 'a \<Rightarrow> bool" (infix "\<prec>" 50) 
  where "x \<prec> y \<equiv> x \<preceq> y \<and> x \<noteq> y"

lemma proper_parthood_irreflexive: "\<not> x \<prec> x"
  by (simp add: proper_part_def)

lemma proper_parthood_asymmetric: "x \<prec> y \<longrightarrow> \<not> y \<prec> x" oops

definition overlap:: "'a \<Rightarrow> 'a \<Rightarrow> bool" ("O")
...
definition sum:: "'a \<Rightarrow> 'a \<Rightarrow> 'a" (infix "\<oplus>" 52)
...
definition product:: "'a \<Rightarrow> 'a \<Rightarrow> 'a" (infix "\<otimes>" 53)
...
definition universe:: "'a" ("u")
  where "u \<equiv> THE z. \<forall> w. w \<preceq> z"

definition difference:: "'a \<Rightarrow> 'a \<Rightarrow> 'a" (infix "\<ominus>" 51)
  where "x \<ominus> y \<equiv> THE z. \<forall> w. w \<preceq> z \<longleftrightarrow> w \<preceq> x \<and> D w y"

definition complement:: "'a \<Rightarrow> 'a" ("\<midarrow>")
  where "\<midarrow> x \<equiv> THE z. \<forall> w. w \<preceq> z \<longleftrightarrow> D w x"

definition general_sum:: "('a \<Rightarrow> bool) \<Rightarrow> 'a" ("\<sigma>")
  where "\<sigma> F \<equiv> THE x. \<forall> y. O y x \<longleftrightarrow> (\<exists> z. F z \<and> O z y)"

abbreviation general_sum_infix:: "('a \<Rightarrow> bool) \<Rightarrow> 'a" (binder "\<sigma>" [8] 9)
  where "\<sigma> x. F x \<equiv> \<sigma> F" --  "general sum or fusion of the Fs"

lemma sum_of_its_proper_parts: "\<exists> y. y \<prec> x \<longrightarrow> x = (\<sigma> z. z \<prec> x)"
  using proper_parthood_irreflexive by blast

definition general_product:: "('a \<Rightarrow> bool) \<Rightarrow> 'a" ("\<pi>")
  where "\<pi> F \<equiv> \<sigma> x. \<forall> y. F y \<longrightarrow> x \<preceq> y"

abbreviation general_product_infix:: "('a \<Rightarrow> bool) \<Rightarrow> 'a" (binder "\<pi>" [8] 9)
...
locale ground_mereology = mereology +
 assumes part_reflexivity: "x \<preceq> x"
 assumes part_antisymmetry: "x \<preceq> y \<longrightarrow> y \<preceq> x \<longrightarrow> x = y"
 assumes part_transitivity: "x \<preceq> y \<longrightarrow> y \<preceq> z \<longrightarrow> x \<preceq> z"

begin

interpretation partial_order: partial_order "(|carrier = A, eq = op =, le = op \<preceq>|)"
  using part_reflexivity part_antisymmetry part_transitivity by unfold_locales auto

lemma "x = y \<longleftrightarrow> x \<preceq> y \<and> y \<preceq> x"
  using part_antisymmetry part_reflexivity by auto

lemma "x = y \<longleftrightarrow> (\<forall> z. z \<preceq> x \<longleftrightarrow> z \<preceq> y)"
  using part_antisymmetry part_reflexivity by blast

lemma "x = y \<longleftrightarrow> (\<forall> z. x \<preceq> z \<longleftrightarrow> y \<preceq> z)"
  using part_antisymmetry part_reflexivity by blast

lemma proper_part_asymmetry: "x \<prec> y \<longrightarrow> \<not> y \<prec> x"
  by (simp add: proper_part_def part_antisymmetry)

lemma proper_part_transitivity: "x \<prec> y \<longrightarrow> y \<prec> z \<longrightarrow> x \<prec> z"
  by (metis proper_part_def proper_part_asymmetry part_transitivity)

lemma "x \<prec> y \<longleftrightarrow> x \<preceq> y \<and> \<not> y \<preceq> x"
  using proper_part_def part_antisymmetry by auto

lemma "x \<preceq> y \<and> y \<prec> z \<longrightarrow> x \<prec> z"
  using proper_part_def proper_part_transitivity by blast

lemma "x \<prec> y \<and> y \<preceq> z \<longrightarrow> x \<prec> z"
  using proper_part_def proper_part_transitivity by blast

lemma overlap_reflexive: "O x x"
  using overlap_def part_reflexivity by blast

lemma parthood_implies_overlap: "x \<preceq> y \<longrightarrow> O x y"
  using overlap_def part_reflexivity by auto

lemma "x \<preceq> y \<and> O x z \<longrightarrow> O y z"
  using overlap_def part_transitivity by blast

lemma "(\<forall> z. z \<preceq> x \<longrightarrow> O z y) \<longleftrightarrow> (\<forall> z. O z x \<longrightarrow> O z y)"
  by (meson overlap_def part_transitivity parthood_implies_overlap)
...
lemma "x \<preceq> y \<and> D y z \<longrightarrow> D x z"
  by (meson disjoint_def overlap_def part_transitivity)

lemma "U x x"
  using underlap_def part_reflexivity by blast

lemma  parthood_implies_underlap: "x \<preceq> y \<longrightarrow> U x y"
  using underlap_def part_reflexivity by auto

lemma "x \<preceq> y \<and> U y z \<longrightarrow> U x z"
  using underlap_def part_transitivity by blast

lemma "(\<forall> z. x \<preceq> z \<longrightarrow> U z y) \<longleftrightarrow> (\<forall> z. U z x \<longrightarrow> U z y)"
  by (metis underlap_def part_transitivity parthood_implies_underlap)
...
proof
  ...
  proof (rule the_equality)
    show "\<And> v. \<forall> w. w \<preceq> v \<longleftrightarrow> (w \<preceq> x \<and> w \<preceq> y) \<Longrightarrow> v = z "
      by (meson antecedent part_antisymmetry part_reflexivity)
  qed
  thus "x \<otimes> y = z"
...
proof
  ...
  proof (rule the_equality)
    show "\<And>z. \<forall>w. (w \<preceq> z) = D w x \<Longrightarrow> z = y"
      using antecedent part_antisymmetry part_reflexivity by blast
  qed
  thus "(\<midarrow> x) = y"
...
lemma proper_weak_supplementation: "(\<forall> x. \<forall> y. x \<prec> y \<longrightarrow> (\<exists> z. z \<prec> y \<and> D z x))"
  by (metis disjoint_def disjoint_symmetric parthood_implies_overlap
proper_part_def weak_supplementation)
...
lemma strong_company: "x \<prec> y \<longrightarrow> (\<exists> z. z \<prec> y \<and> \<not> z \<preceq> x)"
  by (meson disjoint_def parthood_implies_overlap proper_weak_supplementation)
...
lemma proper_parts_principle:  "(\<exists> z. z \<prec> x) \<longrightarrow> (\<forall> z. z \<prec> x \<longrightarrow> z \<preceq> y) \<longrightarrow> x \<preceq> y"
  by (metis disjoint_def overlap_def proper_part_def part_reflexivity strong_supplementation)
...
lemma extensionality: "(\<exists> z. z \<prec> x \<or> z \<prec> y) \<longrightarrow> (\<forall> z. z \<prec> x \<longleftrightarrow> z \<prec> y) \<longrightarrow> x = y"
  by (meson proper_part_def part_antisymmetry proper_parts_principle)

lemma weak_supplementation: "x \<prec> y \<longrightarrow> (\<exists> z. z \<preceq> y \<and> D z x)"
  using proper_part_def part_antisymmetry strong_supplementation by blast

lemma parts_of_parts_overlap: "x \<preceq> y \<longleftrightarrow> (\<forall> z. z \<preceq> x \<longrightarrow> O z y)"
  using disjoint_def part_transitivity parthood_implies_overlap strong_supplementation by blast

lemma part_overlappers_overlap: "x \<preceq> y \<longleftrightarrow> (\<forall> z. O z x \<longrightarrow> O z y)"
  by (meson overlap_def part_transitivity parts_of_parts_overlap)
lemma identity_overlap_eq: "x = y \<longleftrightarrow> (\<forall> z. O x z \<longleftrightarrow> O y z)"
  by (meson disjoint_def overlap_symmetric part_antisymmetry parthood_implies_overlap strong_supplementation)

lemma "x \<preceq> y \<longleftrightarrow> (\<forall> z. D z y \<longrightarrow> D z x)"
  by (metis disjoint_def part_transitivity overlap_def strong_supplementation)

lemma "x \<preceq> y \<longleftrightarrow> (\<forall> z. D z y \<longrightarrow> \<not> z \<preceq> x)"
  by (meson disjoint_def part_transitivity parthood_implies_overlap strong_supplementation)

lemma disjoin_equivalence: "x = y \<longleftrightarrow> (\<forall> z. D z x \<longleftrightarrow> D z y)"
  by (metis disjoint_def proper_part_def parthood_implies_overlap strong_supplementation weak_supplementation)
...
lemma universe_intro: "(\<forall> y. y \<preceq> x) \<longrightarrow> x = u"
  by (simp add: part_antisymmetry the_equality universe_def)
...
proof -
  ...
  proof (rule the_equality)
    show "\<forall> y. O y x \<longleftrightarrow> (\<exists>z. z \<preceq> x \<and> O z y)"
      using overlap_symmetric part_overlappers_overlap by blast
...
proof -
  ...
  proof (rule the_equality)
    show "\<forall>y. O y x = (\<exists>v. (\<forall>j. x \<preceq> j \<longrightarrow> v \<preceq> j) \<and> O v y)"
      by (meson overlap_def part_reflexivity part_transitivity)
    thus "\<And>z. \<forall>y. O y z = (\<exists>v. (\<forall>j. x \<preceq> j \<longrightarrow> v \<preceq> j) \<and> O v y) \<Longrightarrow> z = x"
      by (metis part_antisymmetry parthood_implies_overlap parts_of_parts_overlap)
  qed
...
lemma part_of_first_factor: "O x y \<longrightarrow> x \<otimes> y \<preceq> x"
  using part_reflexivity product_character by blast

lemma part_of_second_factor: "O x y \<longrightarrow> x \<otimes> y \<preceq> y"
  using part_reflexivity product_character by blast
...
lemma "x \<preceq> y \<longrightarrow> x \<otimes> y = x"
  using part_antisymmetry part_reflexivity parthood_implies_overlap product_character by blast

lemma  "O x y \<and> x = x \<otimes> y \<longrightarrow> x \<preceq> y"
  using part_of_second_factor by force
...
lemma proper_part_of_first_factor: "x \<noteq> y \<and> O x y \<longrightarrow> x \<otimes> y \<prec> x \<or> x \<otimes> y \<prec> y"
  by (simp add: part_of_first_factor part_of_second_factor proper_part_def)
...
proof
  ...
  thus "x \<otimes> (y \<otimes> z) = (x \<otimes> y) \<otimes> z"
    using xyz part_antisymmetry part_reflexivity by blast
qed
...
proof fix x y
  ...
  proof cases
    ...
    thus "(\<exists> z. z \<preceq> x \<and> D z y)"
      using part_reflexivity by auto
  next
    assume "\<not> D x y"
    hence "O x y"
    ...
    hence "x \<otimes> y \<prec> x"
      using \<open>\<not> x \<preceq> y\<close> part_reflexivity proper_part_def by auto
    ...
    hence "c \<preceq> x \<and> D c y"
      by (meson disjoint_def overlap_def part_transitivity product)
    thus "\<exists> z. z \<preceq> x \<and> D z y"..
  qed
qed
...
proof
  ...
  with underlapping_sum_character have xy: "(\<forall> w. O w (x \<oplus> y) \<longleftrightarrow> (O w x \<or> O w y))"..
   hence "U (x \<oplus> y) z"
    using underlap_def antecedent part_overlappers_overlap by auto
  ...
  thus "x \<oplus> (y \<oplus> z) = (x \<oplus> y) \<oplus> z"
    using part_overlappers_overlap part_antisymmetry by auto
qed

lemma sums_of_parts_are_parts: "x \<preceq> z \<and> y \<preceq> z \<longrightarrow> x \<oplus> y \<preceq> z"
proof
  ...
  thus "x \<oplus> y \<preceq> z"
    using part_overlappers_overlap \<open>x \<preceq> z \<and> y \<preceq> z\<close> by auto
qed

lemma part_implies_absorption: "y \<preceq> x \<longrightarrow> x = x \<oplus> y"
  by (metis sum_intro part_overlappers_overlap)
...
proof
  ...
  proof (rule the_equality)
    show "\<And> y. \<forall> x. x \<preceq> y \<Longrightarrow> y = z"
      by (simp add: \<open>\<forall>x. x \<preceq> z\<close> part_antisymmetry)
  qed
  thus "u = z"
...
lemma "u \<preceq> x \<longrightarrow> u = x"
  by (simp add: part_antisymmetry universe_character)

lemma "\<not> u \<prec> x"
  by (simp add: part_antisymmetry proper_part_def universe_character)
...
lemma "x \<oplus> u = u"
  using part_implies_absorption sum_commutative universe_character by auto
...
lemma first_summand_inclusion: "x \<preceq> x \<oplus> y"
  by (simp add: part_overlappers_overlap sum_character)

lemma second_summand_inclusion: "y \<preceq> x \<oplus> y"
  using first_summand_inclusion sum_commutative by fastforce

lemma  sum_is_part_iff_summands_are: "x \<oplus> y \<preceq> z \<longleftrightarrow> x \<preceq> z \<and> y \<preceq> z"
  using first_summand_inclusion part_transitivity second_summand_inclusion
sums_of_parts_are_parts by blast

lemma  disjoint_summands_are_proper_parts: "D x y \<longrightarrow> x \<prec> x \<oplus> y \<and> y \<prec> x \<oplus> y"
  by (metis disjoint_def disjoint_symmetric first_summand_inclusion parthood_implies_overlap
proper_part_def second_summand_inclusion)

lemma nonpart_implies_proper_summand: "\<not> x \<preceq> y \<longrightarrow> y \<prec> x \<oplus> y"
  by (metis first_summand_inclusion proper_part_def second_summand_inclusion)

lemma distinct_iff_proper_summand: "x \<noteq> y \<longleftrightarrow> x \<prec> x \<oplus> y \<or> y \<prec> x \<oplus> y"
  using first_summand_inclusion proper_part_def second_summand_inclusion sum_idempotence by auto

lemma absorption_iff_part: "x = x \<oplus> y \<longleftrightarrow> y \<preceq> x"
  by (metis part_implies_absorption second_summand_inclusion)

lemma  disjoint_second_implies_part_first: "(x \<preceq> y \<oplus> z \<and> D x z) \<longrightarrow> x \<preceq> y"
proof
  ...
  thus "x \<preceq> y"
    by (meson antecedent disjoint_def overlap_symmetric part_overlappers_overlap parts_of_parts_overlap)
qed

lemma proper_sum_monotonicity: "x \<prec> y \<and> D y z \<longrightarrow> x \<oplus> z \<prec> y \<oplus> z"
  by (metis absorption_iff_part disjoint_second_implies_part_first proper_part_def sum_commutative 
second_summand_inclusion sum_is_part_iff_summands_are)
...
proof
  ...
  thus "(x \<ominus> y) \<prec> x"
    using \<open>O x y \<and> \<not> x \<preceq> y\<close> disjoint_def part_reflexivity proper_part_def by auto
qed

lemma proper_part_implies_proper_difference: "x \<prec> y \<longrightarrow> y \<ominus> x \<prec> y"
  using overlap_symmetric parthood_implies_overlap proper_difference proper_part_asymmetry
proper_part_def by blast

lemma no_difference_implies_disjoint: "\<not> y \<preceq> x \<and> y \<ominus> x = y \<longrightarrow> D x y"
  using disjoint_def disjoint_symmetric proper_difference proper_part_def by blast
...
proof
  ...
  hence "U x (y \<ominus> x)"
    by (metis \<open>x \<prec> y\<close> proper_part_def underlap_def part_reflexivity)
  ...
  hence "\<forall> w. O w (x \<oplus> (y \<ominus> x)) \<longleftrightarrow> O w y \<or> (O w y \<and> D w x)"
    by (metis \<open>x \<prec> y\<close> difference disjoint_def part_overlappers_overlap overlap_def
overlap_symmetric proper_part_def)
  hence  "\<forall> w. O w (x \<oplus> (y \<ominus> x)) \<longleftrightarrow> O w y"
    by blast
  thus "x \<oplus> (y \<ominus> x) = y"
    by (simp add: part_antisymmetry part_overlappers_overlap)
 qed  
...
proof
  ...
  hence "\<forall> w. w \<preceq> (x \<ominus> (x \<otimes> y)) \<longleftrightarrow> w \<preceq> (x \<ominus> y)"
    using product right disjoint_def overlap_def part_transitivity by smt
...
proof
  ...
  proof
    ...
    hence "\<not> y \<preceq> z"
      using \<open>\<not> x \<preceq> z\<close> part_transitivity by blast
    ...
    hence "\<forall> w. w \<preceq> (x \<ominus> z) \<longrightarrow> w \<preceq> (y \<ominus> z)"
      using part_transitivity \<open>x \<preceq> y\<close> left by blast
...
proof
  ...
  proof (rule the_equality)
    show "\<And>z. \<forall>w. (w \<preceq> z) = D w x \<Longrightarrow> z = a"
      using a part_antisymmetry part_reflexivity by blast
  qed
  hence "(\<midarrow> x) = a"
    using complement_def by auto
...
lemma disjoint_implies_overlaps_complement: "D x y \<longrightarrow> O x (\<midarrow> y)"
  using complement_character disjoint_def parthood_implies_overlap by blast
...
proof
  ...
  hence "\<not> (\<forall> z. z \<preceq> y)"
    using disjoint_def parthood_implies_overlap by blast
...
proof
  ...
  hence "\<forall> y. O y (\<midarrow> x) \<longleftrightarrow> (\<exists> z. (D x z) \<and> O z y)"
    by (meson disjoint_symmetric overlap_symmetric part_overlappers_overlap)
...
proof
  ...
  thus "x \<noteq> u"
    by (meson disjoint_def part_reflexivity parthood_implies_overlap universe_character)
next
  assume "x \<noteq> u"
...
lemma complement_disjointness: "x \<noteq> u \<longrightarrow> D x (\<midarrow> x)"
  using disjoint_symmetric part_reflexivity univ_complement_character by blast
...
proof
  ...
  thus "(x \<oplus> (\<midarrow> x)) = u"
    by (metis \<open>\<forall>w. (w \<preceq> \<midarrow> x) = D w x\<close> disjoint_def part_overlappers_overlap parthood_implies_overlap universe_intro)
qed
...
disjoint_second_implies_part_first first_summand_inclusion part_antisymmetry
...
proof
  ...
  proof
    ...
    hence "w \<otimes> x \<preceq> x \<otimes> (\<midarrow> y)" using prodwx prodxy
      using antecedent disjoint disjoint_def overlap_def part_of_second_factor prod_comp by auto
    hence "w \<otimes> x \<preceq> w \<and> w \<otimes> x \<preceq> x \<otimes> (\<midarrow> y)"
      by (simp add: antecedent part_of_first_factor)
...
proof
  ...
  thus "(x \<oplus> y) \<ominus> x = y"
    by (metis \<open>y \<preceq> x \<oplus> y \<and> D y x\<close> disjoint_second_implies_part_first sum_commutative
part_antisymmetry part_reflexivity)
qed
...
proof -
  ...
  hence  "\<exists> z. \<forall> y. O y z \<longleftrightarrow> (\<exists> x. O x y)"
    by simp
  thus ?thesis
    by (metis overlap_def part_reflexivity)
qed
  ...
  proof
    ...
    proof
      ...
      thus "(\<forall> w. w \<preceq> y \<longrightarrow> (\<exists> v. F v \<and> O v w))"
        by (metis overlap_def part_reflexivity part_transitivity \<open>y \<preceq> z\<close> z)
    next
...
proof
  assume "O x y"
  hence common_part: "\<exists> z. (z \<preceq> x \<and> z \<preceq> y)"
  ...
  hence common_part_sum: "(\<forall> w. w \<preceq> (\<sigma> v. (v \<preceq> x \<and> v \<preceq> y)) \<longleftrightarrow> (\<forall> z. z \<preceq> w \<longrightarrow> (\<exists> v. (v \<preceq> x \<and> v \<preceq> y) \<and> O v z)))"
    using common_part..
  ...
  proof
    ...
    proof
      ...
      hence "(\<forall> t. t \<preceq> w \<longrightarrow> (\<exists> v. v \<preceq> x \<and> v \<preceq> y \<and> O v t))"
        using common_part_sum by simp
      hence "\<forall> t. t \<preceq> w \<longrightarrow> (O t x \<and> O t y)"
        by (meson overlap_symmetric part_overlappers_overlap)
      thus "w \<preceq> x \<and> w \<preceq> y"
        using strong_supplementation part_transitivity disjoint_def by meson
    next
      assume "w \<preceq> x \<and> w \<preceq> y"
      thus "w \<preceq> (\<sigma> v. (v \<preceq> x \<and> v \<preceq> y))"
        using overlap_def part_reflexivity common_part_sum by fastforce
    qed
  qed
  ...
  proof
    ...
    proof
      ...
      have "\<forall> z. z \<preceq> w \<longrightarrow> O z a"
        using left overlap_symmetric part_overlappers_overlap sum by blast
      hence "w \<preceq> a"
        using strong_supplementation disjoint_def by blast
      have "\<forall> v. v \<preceq> w \<longrightarrow> \<not> v \<preceq> b"
        by (metis overlap_def disjoint_def part_transitivity sum left)
      hence "D w b"
    ...
    next
      ...
      thus "w \<preceq> (\<sigma> v. (v \<preceq> a \<and> D v b))"
        using overlap_symmetric parthood_implies_overlap sum by blast
    qed
  qed
    ...
    proof
      ...
      proof
        ...
        thus "(\<forall> z. F z \<longrightarrow> z \<preceq> y)"
          using overlap_symmetric part_overlappers_overlap z by blast
      next
    ...
    proof
      ...
      proof
        ...
        with \<open>F z\<close> have "(z \<preceq> (\<sigma> v. F v))"
          by (metis overlap_symmetric part_reflexivity parts_of_parts_overlap)
        with antecedent show "D z x" 
          by (metis disjoint_def overlap_symmetric part_overlappers_overlap)
      qed
    qed
  next
...
part_overlappers_overlap parthood_implies_overlap z)
  qed
qed
